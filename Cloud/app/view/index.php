...
$paging_query = "
    ...
    `outset`.`Date`, `outset`.`Download`,`outset`.`shape_idx`,
    `user`.`Username`
    FROM `outset`
...
$outset_all = $GLOBALS['db']->prepare($paging_query);
$outset_all->execute([]);
$outset_all_count = $outset_all->rowCount();

$paging_all_block = ceil($outset_all_count / $paging_number);
$paging_block = 5;
...
$ourset = $GLOBALS['db']->prepare($paging_query."LIMIT {$paging_start}, {$paging_number}");
$ourset->execute([]);
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination justify-content-center mb-0">
                        <?php if ($paging_index >= 6) {?>
                            <li class='page-item'><a href='?p=<?= $paging_block_start - $paging_block ?>&search=<?=$_GET['search']?>' class='page-link' style='color: #333333;' tabindex='-1'>Previous</a></li>
                        <?php } ?>
                        <?php for ($i = $paging_block_start; $i < $paging_block_end; $i++) { ?>
                            <?php if ($paging_index == $i) { ?>
                                <li class='page-item'><a href='"?p=<?= $i ?>&search=<?=$_GET['search']?>"' class='page-link' style='color:#fff; background: #555'><?= $i ?></a></li>
                            <?php } else { ?>
                                <li class='page-item'><a href='"?p=<?= $i ?>&search=<?=$_GET['search']?>"' class='page-link' style='color:#333'><?= $i ?></a></li>
                            <?php } ?>
                        <?php } ?>
                        <?php if ($paging_block_start < ($paging_all_block_number * $paging_block - $paging_block) + 1) {?>
                            <li class='page-item'><a href="?p=<?= $paging_block_start + $paging_block ?>&search=<?=$_GET['search']?>" class='page-link' style='color: #333333;'>Next</a></li>
                        <?php } ?>
                    </ul>
                </nav>
            </td>
